...
#
#   IMPORTANT -- THIS SCRIPT MAY SHORTLY BE UNNECESSARY.
#   The root cause of the missing trip expansions has since been found and
#   patched in multi_fishery_trip_summary.R: `previously_interviewed == 0`
#   evaluated to NA and dropped every interview in the six District 11
#   fisheries, and the Drano rows came from the hard-coded "Standard" design.
#   Both are now fixed at source. After a fresh batch run, the batch should
#   produce these fisheries directly and the manual runs become a CHECK on it
#   rather than a substitute for it -- diff the two rather than merging blindly.
#   This script is retained because the committed runs are the only estimates
#   that exist until that re-run happens, and because agreement between the two
#   paths is the cheapest available validation of the patch.
#
# Usage:
...
reconcile_run <- function(run_dir, run_id, fishery_name, study_design,
  ...
  # Mirrors prep_trip_length_interviews() in multi_fishery_trip_summary.R.
  #
  # The original filter, `filter(trip_status == "Complete",
  # previously_interviewed == 0)`, dropped every interview in six of the ten
  # manual-run fisheries because previously_interviewed is entirely NA there and
  # `== 0` yields NA. Confirmed by diagnose_trip_expansion_gaps.R:
  #
  ...
  #
  # Drano is unaffected (612-862 of ~1000 pass), which is why the bug only ever
  # surfaced on the District 11 fisheries. Reproducing the old filter here would
  # have made the integration look successful while changing nothing.

  n_iv <- nrow(interview_angler_types)
  n_fail <- sum(interview_angler_types$angler_final == "fail", na.rm = TRUE)
  has_prev <- "previously_interviewed" %in% names(interview_angler_types)
  n_prev_na <- if (has_prev) sum(is.na(interview_angler_types$previously_interviewed)) else NA_integer_

  prev_int_status <- dplyr::case_when(
    !has_prev          ~ "absent",
    n_prev_na == n_iv  ~ "not_collected",
    n_prev_na > 0      ~ "partial",
    TRUE               ~ "collected"
  )
  if (prev_int_status != "collected") {
    cli::cli_alert_warning(
      "  {.val {fishery_name}}: previously_interviewed {prev_int_status} \\
       ({n_prev_na}/{n_iv} NA); coalesced to 0 so the fishery can expand."
    )
  }
  if (n_fail > 0) {
    cli::cli_alert_warning(
      "  {.val {fishery_name}}: {n_fail} interview{?s} with angler_final = \\
       'fail' excluded from trip length."
    )
  }
  ...
  mean_trip_length_monthly <- interview_angler_types |>
    ...
    dplyr::filter(
      ...
      angler_final != "fail",
      dplyr::coalesce(
        if (has_prev) as.numeric(previously_interviewed) else 0, 0
      ) == 0
    ) |>
  ...
  trip_rows <- total_effort_monthly |>
    ...
    dplyr::mutate(
      ...
      # Matches the patched multi_fishery_trip_summary.R: an estimated zero
      # effort means zero trips, not unknown trips.
      ...
      prev_int_status = .env$prev_int_status,
      source          = "manual",
      run_id          = .env$run_id
    ) |>
    dplyr::select(
      ...
      n_trip_length_obs, mean_group_size, sd, total_trips_est,
      trip_expansion, pe_period, prev_int_status, source, run_id
    )
