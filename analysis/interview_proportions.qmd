...
- **Location (bank/boat)** for these rivers already comes from `angler_final`
  ...
  `R3_external`) — `pst_fw_effort_assembly.R` reads it directly as
...
This is the hand-off to `analysis/pst_fw_effort_assembly.R`. Everything above
...
`multi_fishery_trip_summary.csv`. An unnormalized join silently drops every row.
...
`pst_fw_effort_assembly.R` falls back to block-pooled proportions below
...
The interview pull and `multi_fishery_trip_summary.R` draw on the same creel
...
this document produces. A parent Quarto document covering methods across all
four regional blocks would be the right home for it, with this file reduced to
the Track B derivation it actually performs.
:::
