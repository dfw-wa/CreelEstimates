...
location_basis_label <- function(x) {
  ...
  est <- case_when(
    str_detect(x, "assumed: bank only") & str_detect(x, "guided logbook trips placed in boat") ~
      "Assumed bank only for unguided trips (no data available to assign them to boat location); guided logbook trips placed in boat",
...
if (!is.null(deliverable_trips)) {
  ...
  note <- c(
    ...
    "## Bank and boat",
    "",
    "Where a creel survey measured bank and boat effort separately, that split is used as measured. Elsewhere, including all estimates expanded from catch record cards, the split is borrowed: from a creel survey on the same river (same year if available, else other years); else from creel interviews on the same river, where at least 100 interviews recorded bank or boat (the months weighted by that river's reported salmon harvest, so salmon-season fishing sets the split); else from the region's creel surveys; else from all creel surveys statewide. Rivers with no creel survey or interview data were reviewed individually by WDFW staff familiar with them: where a river is known to be fished from boats, the boat share was set by that professional judgement; otherwise the river is assumed bank only, as there is no data available to assign trips to boat location. Guided logbook trips on bank-only rivers are still placed in boat, as guided trips are elsewhere. Rivers not open for salmon were set to zero trips. Guided trips are placed in boats at the rate creel interviews show for guided salmon anglers (about 96% overall; river-specific where at least 20 guided interviews exist). Unguided trips make up the rest of each bank and boat total.",
    "",
    ...
    glue("- Of which from same-river creel interviews: {share(dt$`Location Basis` == 'Estimated — same-river creel interviews')}% of all trips."),
    glue("- Location share set by professional judgement (rivers without creel data): {share(dt$`Location Basis` == 'Set by professional judgement')}% of all trips."),
    glue("- Location assumed bank only (no data available to assign trips to boat location; guided logbook trips on these rivers are placed in boat): {share(str_starts(dt$`Location Basis`, 'Assumed bank only'))}% of all trips."),
    "",
