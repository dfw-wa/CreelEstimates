...
# Location: analysis/pst_fw_effort_assembly.R
#
...
#   1. analysis/parse_crc_freshwater_harvest.R      -> crc_freshwater_harvest_*.csv
#   2. analysis/multi_fishery_creel_summary.R       -> multi_fishery_creel_trips.csv
#                                                      multi_fishery_creel_harvest.csv
#   3. analysis/mid_columbia_yakima_creel_ingestion.R -> mid_columbia_yakima_creel_summary.csv
#   4. analysis/interview_proportions.qmd           -> interview_mode_location_props.csv
#   5. analysis/pst_fw_effort_assembly.R            <- THIS SCRIPT
#
# Design rules:
...
source(here("R_scripts", "pst_p2_block_ratio.R"))
...
manifest  <- read_csv(file.path(PST_DIR, "pst_input_manifest.csv"),
                      show_col_types = FALSE)
crosswalk <- read_csv(file.path(PST_DIR, "pst_river_block_crosswalk.csv"),
                      show_col_types = FALSE)
...
if (n_mainstem > 0) {
  log_gap("crosswalk", "ColumbiaMainstem", "note",
          glue("{n_mainstem} ColumbiaMainstem crosswalk row(s) filtered at load. ",
               "Run R_scripts/patch_crosswalk_areas.R to remove them permanently."))
...
read_merged_creel <- function(file, source_id) {
  d <- read_if(file.path(OUT_DIR, file), source_id,
               detail = "run analysis/multi_fishery_creel_summary.R first")
...
ingest_creel_pe <- function() {
  ...
  d |>
    ...
    left_join(crosswalk |> filter(source_id == "creel_pe") |>
...
ingest_r3_external <- function() {
  ...
  d |>
    filter(!is.na(total_trips_est)) |>
    left_join(crosswalk |> filter(source_id == "R3_external") |>
                select(fishery_name, river_label, block),
              by = "fishery_name") |>
    mutate(
      location       = coalesce(angler_final, "unknown"),
...
apply_track_b <- function(trips) {
  props <- read_if(file.path(OUT_DIR, "interview_mode_location_props.csv"),
                   "interview_prop",
                   detail = paste("run analysis/interview_proportions.qmd and",
...
if (!is.null(p2x)) {
  ...
  log_gap("p2_expansion", NA, "blocker",
          "P2 did not run - no CRC table or no CRC/creel area overlap.")
}
...
# the output are driven by the same table rather than diverging.
coverage <- crosswalk |>
...
if (nrow(gaps)) {
  message(glue("\n{sum(gaps$severity == 'blocker')} blockers, ",
               ...
               "-> analysis/outputs/pst_fw_gap_register.csv"))
}
...
# ==============================================================================
# ---- 8. Excel export for Jim's status update ---------------------------------
# ==============================================================================
# One workbook, one tab per artifact, so a non-R reader can open it and see
# what the pipeline currently supports without running anything. This is a
# STATUS UPDATE, not the Northern Economics deliverable - the filename and the
# cover tab both say so, and every gap/blocker still visible in the console
# summary above is visible here too, not smoothed over for presentation.
#
...
#
# Missing upstream inputs (interview props not run, P2 not wired, etc.) are
# handled the same way as everywhere else in this script: log and skip the
# sheet, never fabricate one. [R2]
...
if (!is.null(p2x) && nrow(p2x$loo_summary) > 0) {
  coverage_summary <- coverage_summary |>
    left_join(p2x$loo_summary |> select(block, p2_median_error_pct = median_ape),
              by = "block") |>
...
## 8b. Deliverable shape (river x year x mode x location) ---------------------
...
## 8d. P2 evidence: ratios + leave-one-out -------------------------------------

if (!is.null(p2x)) {
  if (nrow(p2x$ratios) > 0) {
    add_sheet(wb, "P2 Ratios", p2x$ratios,
             title = "Block-year expansion ratio (creel trips / CRC salmon) — CRC-denominated")
  }
  if (nrow(p2x$loo_summary) > 0) {
    add_sheet(wb, "P2 Validation (LOO)", p2x$loo_summary,
             title = "Leave-one-out: predict each surveyed area from the rest — median_ape is the number to quote")
  }
  if (nrow(p2x$donors) > 0) {
    add_sheet(wb, "P2 Donor Areas", p2x$donors,
             title = "Areas with BOTH a creel survey and CRC harvest — the evidence base P2 is built from")
  }
} else {
  log_gap("xlsx_export", NA, "note", "P2 did not run this session; P2 tabs omitted from the workbook.")
}

## 8e. CRC vs. creel bias -------------------------------------------------------
...
## 8g. Provenance (collapsed, not the full row-level ledger) -------------------
