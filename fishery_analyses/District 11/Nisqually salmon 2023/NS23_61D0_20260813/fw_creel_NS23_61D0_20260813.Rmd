...
```{r, dynamic_total_salmon_catch_group_setup, message=FALSE}
# Dynamic "all kept salmon" catch group -------------------------------------
#
...
# rows). Used for fisheries (Puyallup/Carbon, Nisqually, Drano Lake) where
# we want a single "all kept salmon" catch group built from whatever
# life-stage/fin-mark values are actually present in the data, rather than
# the hard-coded params$est_catch_groups default -- see the dwg_fetch chunk
# below, where this overwrites params$est_catch_groups once dwg is available.

# Species eligible for the PST salmon rollup. Steelhead, trout, char, and
# other gamefish are excluded by design. Matching is EXACT against the
# values in dwg$catch$species (not regex) so a DB-side rename surfaces as a
# warning rather than silently dropping fish.
...
# observed category value (guards against a value like
# "Bull Trout|Dolly Varden" silently changing the meaning of the pattern).
REGEX_METACHARS <- "[.\\\\+*?\\[\\]^$(){}=!<>|:-]"

#' Build a single "all kept salmon" est_catch_groups row from observed data
#'
#' @param dwg_catch  dwg$catch as returned by fetch_dwg()
...
#' @return a one-row data.frame in the shape est_catch_groups expects
#'   (species, life_stage, fin_mark, fate), or NULL (with a warning) if no
#'   PST salmon species / no harvested records are present.
build_total_salmon_catch_group <- function(dwg_catch,
  ...
  # internally, so the values enumerated here match what it will
  # str_detect() against.
  cat_std <- dwg_catch |>
    dplyr::mutate(
      dplyr::across(
  ...
  if (length(bad_vals) > 0) {
    cli::cli_abort(
      c("Category value(s) contain regex metacharacters and cannot be \\
         safely assembled into the catch-group pattern.",
        "x" = "{.val {bad_vals}}",
...
```{r reaggregate_sections, cache=params$enable_cache, results='hide', dependson="dwg_fetch", cache.extra=param_hash}
# aggregate section 2 and 3, assigning all of section 3 to 2
dwg$effort <- dwg$effort |>
  mutate(
    section_num = case_when(
    section_num %in% c(1,2,3,4) ~ 1,
    # ,
...
# Overwrite the hard-coded YAML est_catch_groups with a single "all kept
# salmon" group built dynamically from this fishery's own dwg$catch. See
# build_total_salmon_catch_group() in the dynamic_total_salmon_catch_group_setup
# chunk above. Aborts if no PST salmon / no Kept records are present, since a
# silent empty catch group would produce a misleadingly clean report.
params$est_catch_groups <- build_total_salmon_catch_group(
  dwg_catch    = dwg$catch,
  fishery_name = params$fishery_name
)

if (is.null(params$est_catch_groups)) {
  stop(
...
The following catch groups of fish were selected for generating catch estimates. This reflects the dynamically-built "all kept salmon" group (see `dynamic_total_salmon_catch_group_setup` and `dwg_fetch` above), not the hard-coded YAML default shown in the report parameters.
```{r, echo = FALSE}
...
if (params$export %in% c("database", "local") && !params$model_used %in% c("PE only", "BSS only", "Both models" )) {
  ...
  creel_estimates <- creelutils::transform_estimates(params, transformed_pe_data, transformed_bss_data)  
