...
inputs_bss <- 
  ...
  map(
    ~prep_inputs_bss(
      ...
      study_design = resolved_params$study_design, 
      priors = c(
# Half-Student-t regularizing priors. Degrees of freedom (df) controls tail geometry and adjust scale to set the range of variance expected. Larger scale settings are less informative and mostly affect data-poor strata. Larger DF settings shrink tails:
  # Catch process error (sigma_eps_C) - CPUE daily fluctuations
        value_student_t_df_sigma_eps_C = 4,       
   ...
   # Effort overdispersion (sigma_r_E) - between-angler effort variation 
        value_student_t_df_sigma_r_E = 4,         
        value_student_t_scale_sigma_r_E = 0.5,
   # Catch overdispersion (sigma_r_C) - between-angler CPUE variation
        value_student_t_df_sigma_r_C = 4,
        value_student_t_scale_sigma_r_C = 1.5,
...
# Other priors:
        ...
        value_lognormal_sigma_b = 1, # Bias multiplier on index counts
        value_normal_sigma_B1 = 5, # weekend/weekday fixed effect on log effort
        value_normal_mu_mu_C = {
          cg <- dwg_summ$interview |> filter(est_cg == .x)
          log(sum(cg$fish_count) / sum(cg$fishing_time_total))
